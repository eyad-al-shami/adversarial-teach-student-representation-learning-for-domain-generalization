...
def merge_args_cfg(cfg, args):
    ...
    if args.logger:
        ...
        if (args.experiment_name and args.experiment_name.lower() == "hp"):
            ...
            # change the the experiment name to be cross-platform folder name compatible (no colons)
            cfg.LOGGING.EXPERIMENT_NAME = f"A_{cfg.MODEL.AUGMENTER.LR}-TAU_{cfg.MODEL.TEACHER.TAU}-TW_{cfg.MODEL.TEACHER.WARMUP_LR}-S_{cfg.MODEL.STUDENT.LR}-{utils.get_readable_date_time()}"
        else:
